...
def set_activation_rank_logit_average(
    ...
    d_mlp, d_head = spec["d_mlp"], spec["d_head"]
    ...
    for batch_start in range(0, clean_sample_size, batch_size):
        ...
        with model.trace(remote=remote) as tracer:
            with tracer.invoke(batch_clean_tokens) as invoker:
    ...
    for batch_start in range(0, corrupt_sample_size, batch_size):
        ...
        with model.trace(remote=remote) as tracer:
            #with tracer.invoke(batch_corrupt_tokens):
            with tracer.invoke(batch_corrupt_tokens) as invoker:
                for layer in grouped_components.keys():
                    ...
                    if heads_or_neurons == "heads":
                        interv_attn_output = accessor.layers[layer].attention.output.unwrap().input[:, -1]
                        interv_reshaped = interv_attn_output.reshape(current_batch_size, n_heads, d_head)
                        device = interv_reshaped.device
                        interv_reshaped[:, component_index_list, :] = scaling_factor * clean_act_mean[layer, component_index_list, :].to(device) 
                    elif heads_or_neurons == "neurons":
                        interv_mlp_output = accessor.layers[layer].mlp.down_proj.unwrap().input[:, -1]
                        interv_mlp_output[:, component_index_list] = scaling_factor * clean_act_mean[layer, component_index_list].to(device) 
                    else:
                        raise ValueError(f"heads_or_neurons {heads_or_neurons} not supported")
                ...
                #interv_answer_logits = accessor.lm_head.unwrap().output[:, -1][torch.arange(current_batch_size), batch_answer_tokens].save()
                #interv_logits = model.output.logits[:,-1].save() 
                interv_logits = model.lm_head.output[:, -1].save() 
...
def set_activation_patching_generation(
    model: LanguageModel, batch_size: int = 8,
    grouped_components: dict[int, list[int]] = None,
    prompts_patch_from: list[str] = None,
    prompts_patch_to: list[str] = None,
    remote: bool = False,
    scaling_factor: float = 1,
    n_generate_tokens: int = 1,
    intervene_positions: list[int] = None,  # generation step indices (0-indexed) to apply patch
) -> list[str]:
    """ 
    Patch with the average of activatio from one set of tokens/prompts, then autoregressively generate tokens.

    Prompts are tokenized per batch (padding fits each batch only), so padding does not depend on
    global max length across the full dataset.

    Args:
        model: Language model to analyze
        prompts_patch_from: Prompt strings for the patch-from (clean) activations
        prompts_patch_to: Prompt strings to patch and generate from
        grouped_components: a dict where a list of component_index of a particular layer are grouped under its layer key. 
        batch_size: Size of each processing batch
        remote: Whether to run model remotely
        n_generate_tokens: Number of tokens to generate after patching
        intervene_positions: Which generation steps (0-indexed) to apply the patch. Defaults to [0] (only first step).
       
    Returns:
        decoded_outputs: List of decoded outputs (length: sample_size_patch_to)
    """
    if intervene_positions is None:
        intervene_positions = [0]

    intervene_positions = set(intervene_positions)

    tokenizer = model.tokenizer

    def _tokenize_batch(texts: list[str]) -> torch.Tensor:
        return tokenizer(
            texts,
            padding=True,
            padding_side="left",
    ...
    # Get model specs
    spec = get_model_specs(model)
    n_layers, n_heads, d_head = spec["n_layers"], spec["n_heads"], spec["d_head"]

    sample_size_patch_from = len(prompts_patch_from)
    sample_size_patch_to = len(prompts_patch_to)

    # ── Run & cache activation ───────────────────────────────────────────────
    clean_act = torch.empty(sample_size_patch_from, n_layers, n_heads, d_head)

    for batch_start in range(0, sample_size_patch_from, batch_size):
        batch_end = min(batch_start + batch_size, sample_size_patch_from)
        current_batch_size = batch_end - batch_start

        batch_clean_tokens = _tokenize_batch(prompts_patch_from[batch_start:batch_end])
    ...
    assert clean_act.shape == (sample_size_patch_from, n_layers, n_heads, d_head)
    clean_act_mean = clean_act.mean(dim=0)  # (n_layers, n_heads, d_head)

    del clean_act
    free_unused_cuda_memory()

    # ── Patching + multi-token generation ─────────────────
    print("batch_size", batch_size)
    all_generated_tokens = []

    for batch_start in tqdm(range(0, sample_size_patch_to, batch_size), desc="Running intervention"):
        batch_end = min(batch_start + batch_size, sample_size_patch_to)
        current_batch_size = batch_end - batch_start

        batch_tokens_patch_to = _tokenize_batch(prompts_patch_to[batch_start:batch_end])

        accessor_config = get_accessor_config(model)
        accessor = ModelAccessor(model, accessor_config)

        device = next(model.parameters()).device
        current_tokens = batch_tokens_patch_to.clone().to(device)  # (batch, seq_len), grows each step
        batch_generated = []

        for gen_step in range(n_generate_tokens):
            with model.trace(remote=remote) as tracer:
                with tracer.invoke(current_tokens) as invoker:

                    # Only patch at specified generation steps
                    if gen_step in intervene_positions:
                        for layer in grouped_components.keys():
                            component_index_list = grouped_components[layer]
                            interv_attn_output = accessor.layers[layer].attention.output.unwrap().input[:, -1]
                            interv_reshaped = interv_attn_output.reshape(current_batch_size, n_heads, d_head)
                            device = interv_reshaped.device
                            interv_reshaped[:, component_index_list, :] = (
                                scaling_factor * clean_act_mean[layer, component_index_list, :].to(device)
                            )

                    interv_logits = model.lm_head.output[:, -1].save()  # (batch, vocab)

            # Greedy decode next token
            next_tokens = interv_logits.argmax(dim=-1, keepdim=True)  # (batch, 1)

            # Append to running sequence for next step
            current_tokens = torch.cat([current_tokens, next_tokens], dim=-1)
            batch_generated.append(next_tokens.to('cpu'))

            del interv_logits
            free_unused_cuda_memory()

        # Stack generation steps: (batch, n_generate_tokens)
        batch_generated = torch.cat(batch_generated, dim=-1)
        all_generated_tokens.append(batch_generated)

    # (sample_size_patch_to, n_generate_tokens)
    all_generated_tokens = torch.cat(all_generated_tokens, dim=0)

    decoded_outputs = model.tokenizer.batch_decode(all_generated_tokens, skip_special_tokens=True)  
...
def get_rank_logit_acc_raw_result_dict_scaling_average(
    model: LanguageModel,  batch_size: int = 20,
    grouped_components: dict[int, list]=None,
    all_clean_tokens: torch.Tensor = None,
    all_corrupt_tokens: torch.Tensor = None,
    all_answer_tokens: torch.Tensor = None,
    scaling_factors: list[float]=None,
    heads_or_neurons: str="heads",
    prompt_index_list_2_be_fixed: list[int]=None,
) -> tuple[dict[int, float], dict[int, float], dict[int, float]]:
    """
    Get the reciprocal rank and accuracy across different scaling factors
    Args:
       
    """
    rank_dict = {}
    acc_dict = {}
    fixed_prompt_index_list_dict = {}

    for scaling_factor in scaling_factors:
        interv_ranks = set_activation_rank_logit_average(
            model=model, batch_size=batch_size,
            all_clean_tokens=all_clean_tokens,
            all_corrupt_tokens=all_corrupt_tokens,
            all_answer_tokens=all_answer_tokens,
            ...
            scaling_factor=scaling_factor,
            heads_or_neurons=heads_or_neurons)
        
        interv_acc = compute_top_k_accuracy(interv_ranks.to('cpu'), 1)
        reciprocal_rank = [1 / (x+1) for x in interv_ranks] # +1 to avoid division by zero
        rank_dict[scaling_factor] = sum(reciprocal_rank) / len(reciprocal_rank)
        acc_dict[scaling_factor] = interv_acc
        if prompt_index_list_2_be_fixed is not None:
            fixed_prompt_index = np.where(np.array(interv_ranks.to('cpu')) < 1)[0] # Get the index of fixed prompts
            fixed_prompt_index_ori = np.array(prompt_index_list_2_be_fixed)[fixed_prompt_index]
            fixed_prompt_index_list_dict[scaling_factor] = fixed_prompt_index_ori
    ...
    if prompt_index_list_2_be_fixed is not None:
        result_dict["fixed_prompt_index_list_dict"] = fixed_prompt_index_list_dict
